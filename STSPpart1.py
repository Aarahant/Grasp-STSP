...
### Reads normal problems: Changes Distance measurement and Feasibility checker
import sys
import re
import math
import time
...
def CandidateList(PInicial,Puntos,TimeLink):
    ...
    for PFinal in Puntos:
        if PFinal[4]:
            ...
            if Cost <= (RTime + TimeLink*2) and Cost > 0: # Intento de limitar la lista
                Razon = PFinal[2] / Cost
                Ratio = []
...
def decide(Place1, Place2, Places, TimeLink):
    ...
    best1 = 0
    for r1 in Ratio1:
        bestIndex1 = Ratio1[best1][3]
        P1New = Places[bestIndex1]
        # Profit1 = Ratio1[best1][0]
        Time1 = Ratio1[best1][1]
        best2 = 0
        for r2 in Ratio2:
            bestIndex2 = Ratio2[best2][3]
            P2New = Places[bestIndex2]
            # Profit2 = Ratio2[best1][0]
            Time2 = Ratio2[best2][1]
            if P2New != P1New:
                ...
                for NewPlace in RestrictedPlaces:
                    ...
                    if Euclidean(allPlaces[Place],allPlaces[NewPlace[3]]) <= Radius:
                        ...
                        if NewCost <= Time:
                            ...
                            if NewProfit > bestProfit and [NewSol,NewCost,NewProfit] not in PossibleSol and NewSol.count(NewPlace) < 2:  # Nuevo!!
                                # print(f'Spacing: {NewSol}, Cost {NewCost} , Profit {NewProfit}') ### temporal
                                improvingSolution = NewSol
...
#             LugaresFiltrados = []
#             LugaresFiltrados = CandidateList(lugares[Lugar],listaNuevosSitios, AvrgDist)
...
def solution(lugares):
    ...
    written += 'The selected path is '+ str(AnsLugares) +'\n'
    ...
    spacing(AnsLugares, lugares, TProfit, AvrgDist)
    ...
    for neighbor in sOrderProfit:
        vecino = neighbor[0]
        inserting(vecino, lugares) # cambia bestImproving
    ...
    regresar = [TProfit, Cost, AnsLugares, HeuristicTime, mejorGanancia, impCost, bestImproving, TotalTime, improvementPercent]
    return regresar
    #imprimir en script
    # print(f'{TProfit}:{Cost}:{HeuristicTime}:{mejorGanancia}:{impCost}:{TotalTime}:{improvementPercent}')

# Main
def main(argumento, alfa):
    global comienzo
    global a
    global written
    a = alfa
